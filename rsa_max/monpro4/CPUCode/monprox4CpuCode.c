...
/* Montgomery's reduction */
	...
	u32 a1=x_ptr[0]; /* Set scalar input */
	u32 a2=x_ptr[0]; /* Set scalar input */
	u32 a3=x_ptr[0]; /* Set scalar input */
	u32 a4=x_ptr[0]; /* Set scalar input */

	printf("Running DFE\n");
	t = RDTSC();
	for(int i=0; i<NUM_OF_DIGITS;i++)
		monprox4(DATALENGTH/4, a1, a2, a3, a4,  x_ptr, &x_ptr[DATALENGTH/4], &x_ptr[DATALENGTH/2],&x_ptr[(DATALENGTH/4)*3], data_out_x,  &data_out_x[DATALENGTH/4], &data_out_x[DATALENGTH/2],&data_out_x[(DATALENGTH/4)*3], data_out_y, &data_out_y[DATALENGTH/4], &data_out_y[DATALENGTH/2],&data_out_y[(DATALENGTH/4)*3]);
	t = RDTSC() - t;
