...
u32* monPro(u32* a, u32* b, u32* n, u32 n0_pr){
	...
	//u32 a[2]={0x00000000,0x00010000}; // 0x00010000 = 65536dec, 2^16
	//u32 b[2]={0x00000000,0x00010000};

	//u32 n[NUM_OF_DIGITS+1]={311u,2u,1u};//={0x00000000,0x00010000}; //fix me
	//u32 n0_pr=209; //fix me
...
///////////////////////////////////  Kernel  ////////////////////////////////////////////////////////
	for (i=0; i<s; i++){
		...
		for (j=0; j<s; j++) {
			...
			pp1 = ((a[i] << 16) >> 16) * ((b[j] << 16) >> 16);  
			pp2 = ((a[i] << 16) >> 16) *(b[j] >> 16);
			pp3 = (a[i] >> 16) * ((b[j] << 16) >> 16);
			pp4 = (a[i] >> 16) * (b[j] >> 16);
			...
			sum = x + carry; 	
			sum_1 = sum + t[i+j];						
			carry =((sum < carry) ? 1 : 0) + ((sum_1 < t[i+j]) ? 1 : 0)+y;			
			t[i+j] = sum_1;			
...
u32 borrow, diff = 0;
	...
	if (borrow == 0) return v; //v[0]...v[s-1] is result
	else return u;             //u[0]...u[s-1] is result
